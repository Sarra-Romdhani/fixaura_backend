...
async updatePrestataire(
  ...
  const updateData: Partial<Prestataire> = {};
  const uploadDir = './uploads/profiles'; // Use lowercase 'uploads'
  if (!fs.existsSync(uploadDir)) {
    fs.mkdirSync(uploadDir, { recursive: true });
  }
  const isMultipart = await request.isMultipart();
  ...
  try {
    for await (const part of parts) {
      if (part.type === 'file' && part.fieldname === 'image') {
        ...
        // Delete old image if it exists
        if (prestataire.image) {
          const oldImagePath = join(__dirname, '..', '..', prestataire.image); // Adjust path resolution
          if (fs.existsSync(oldImagePath)) {
            fs.unlinkSync(oldImagePath);
            console.log(`Deleted old image: ${oldImagePath}`);
          }
        ...
        updateData.image = `/uploads/profiles/${filename}`; // Use lowercase 'uploads'
      } else if (part.type === 'field' && part.fieldname) {
        const value = part.value;
        if (value !== undefined) {
          switch (part.fieldname) {
