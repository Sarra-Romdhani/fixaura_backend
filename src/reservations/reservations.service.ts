...
async sendEmail(to: string, subject: string, text: string): Promise<boolean> {
  ...
  //       .orFail(new NotFoundException('Réservation non trouvée'))
  //       .lean()
  //       .exec();
  ...
  //       id_client: populated.id_client.toString(),
  //       id_prestataire: populated.id_prestataire.toString(),
...
async getReservationById(id: string): Promise<Reservation> {
  ...
  async getReservationsByClient(id_client: string): Promise<Reservation[]> {
    return this.reservationModel.find({ id_client }).exec();
  }

  async getCompletedReservationsByClient(id_client: string): Promise<Reservation[]> {
    return this.reservationModel.find({ id_client, status: 'completed' }).exec();
  }

  async getNonCompletedReservationsByClient(id_client: string): Promise<Reservation[]> {
    return this.reservationModel.find({ id_client, status: { $ne: 'completed' } }).exec();
  }
  ...
  async cancelReservation(id: string): Promise<{ canceledReservation: Reservation; matchingReservation?: Reservation }> {
    if (!isValidObjectId(id)) {
